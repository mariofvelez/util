...
public class Transform2d {
	 ...
	 */
	public void drawImage(Graphics2D g2, Image img, Vec2d position, Vec2d dimensions)
	{
		atx.setToTranslation((int) position.x, (int) position.y);
		atx.rotate(tx.x, tx.y);
		atx.scale(dimensions.x/img.getWidth(null) * tx.length(),
				  Math.abs(dimensions.y/img.getHeight(null) * ty.length()));
