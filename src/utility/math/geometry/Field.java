...
public class Field extends Canvas
	...
	public Field(Dimension size) throws Exception {
		...
		img = new BufferedImage(900, 600, BufferedImage.TYPE_INT_ARGB);
		width = img.getWidth();
		height = img.getHeight();
		img_data = ((DataBufferInt) img.getRaster().getDataBuffer()).getData();
		
		buffer = new BufferedImage(900, 600, BufferedImage.TYPE_INT_ARGB);
		...
		try {
			...
			for(int i = 0; i < shapes.length; i++)
			{
				for(int j = 0; j < shapes.length; j++)
				{
					if(i==j)
						continue;
					Vec2d normal = Geometry.intersectionNormal(shapes[i], shapes[j]);
				...
				shapes[i].debugDraw(g2, is_intersected[i]);
			}
			s.debugDraw(g2, true);
