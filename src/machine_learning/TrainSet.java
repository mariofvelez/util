...
public class TrainSet {
    ...
    public TrainSet extractBatch(int size) {
        if(size > 0 && size <= this.size()) {
            ...
            ArrayList<Integer> ids = new ArrayList<Integer>();//[size];//NetworkTools.randomValues(0,this.size() - 1, size);
            for(int i = 0; i < size; i++)
            {
            	int a = Utils.randInt(0, this.size()-1);
            	while(ids.contains(a))
            		a = Utils.randInt(0, this.size()-1);
            	ids.add(a);
            }
            for(Integer i:ids) {
